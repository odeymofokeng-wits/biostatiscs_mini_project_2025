...
#------------------------------------------------------------
# 3) Copula-Graphic Estimator with compound.Cox
#------------------------------------------------------------
...
# Plot sensitivity for treatment group (Figure 3)
x_breaks <- c(-0.02, tau_values)
x_labels <- c("KM", as.character(tau_values))
ggplot(sensitivity_results, aes(x = Tau_display, y = S1_Bias, color = Method)) +
  geom_point(size = 3) +
  geom_line(aes(group = Method), linewidth = 1) +
  geom_errorbar(aes(ymin = S1_Bias_LCI, ymax = S1_Bias_UCI), width = 0.05) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_x_continuous(breaks = x_breaks, labels = x_labels, limits = c(-0.1, 0.9)) +
  scale_color_manual(values = c("KM" = "purple", "Copula-Graphic" = "red")) +
  labs(x = "Kendall's Tau (or KM)", y = "Bias (S(10) - True S(10))") +
  theme_light() + theme(panel.background = element_rect(fill = "white")) + theme(legend.position = "top")
ggsave("Figure3_Sensitivity_Bias.png")
